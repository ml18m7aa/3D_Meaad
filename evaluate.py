...
def main():
    ...
    subject_ids = list()
    for case_folder in glob.glob("prtV/*"):
    ...
    #df1 = pd.DataFrame.from_records(rows1, columns=header, index=subject_ids)
    #df2 = pd.DataFrame.from_records(rows2, columns=header, index=subject_ids)

    #df2.to_csv("./prtV/specificity.csv")
    ...
    plt.savefig("validation_scores_100boxplot.png")
    plt.close()
