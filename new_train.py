...
#     subject_ids = list()
#     for subject_dir in glob.glob(os.path.join('/home/aj/Music/3DUnetCNN-master (copy)/ALL_files/', "preprocessed", "*", "*")):
#         print(subject_dir)
#         # quit()
#         subject_ids.append(os.path.basename(subject_dir))
#         name_data = subject_dir.split('/')[-1]
#
#         subject_files = list()
#         for modality in config["training_modalities"] + ["seg"]:
#             base_name = name_data + '_' + modality
#             subject_files.append(os.path.join(subject_dir, base_name + ".nii.gz"))
...
def fetch_training_data_files(return_subject_ids=False):
    ...
    for subject_dir in glob.glob(
            os.path.join('path of dataset folder', "preprocessed", "*", "*")):
        print(subject_dir)
        # quit()
        subject_ids.append(os.path.basename(subject_dir))
        name_data = subject_dir.split('/')[-1]
...
def main(overwrite=False):
    ...
    if not overwrite and os.path.exists(config["model_file"]):
        ...
        model = is_model(input_shape=config["input_shape"], n_labels=config["n_labels"],
